...
import { Minus, Plus, Trash2, ShoppingBag, ArrowRight, CreditCard, Check, Truck, Loader2 } from "lucide-react";
...
const GEO_DATA: Record<string, Record<string, Record<string, string[]>>> = {
  "India": {
      "Gujarat": {
          ...
          "Amreli": ["Amreli", "Babra", "Bagasara", "Dhari", "Jafrabad", "Khambha", "Kunkavav", "Lilia", "Rajula", "Savarkundla"],
          ...
          "Bharuch": ["Bharuch", "Amod", "Ankleshwar", "Hansot", "Jambusar", "Netrang", "Vagra", "Valia", "Zagadia"],
          ...
          "Gir Somnath": ["Gir Gadhada", "Kodinar", "Sutrapada", "Talala", "Una", "Veraval (Patan-Veraval)"],
          "Jamnagar": ["Jamnagar", "Dhrol", "Jamjodhpur", "Jodiya", "Kalavad", "Lalpur"],
          "Junagadh": ["Junagadh City", "Junagadh Rural", "Bhesan", "Keshod", "Malia Hatina", "Manavadar", "Mangrol", "Mendarda", "Vanthali", "Visavadar"],
          "Kheda": ["Kheda", "Nadiad", "Balasinor", "Dakhor", "Galteshwar", "Kapadvanj", "Kathlal", "Mahudha", "Matar", "Mehmedabad", "Thasra", "Vaso"],
          ...
          "Patan": ["Patan", "Chanasma", "Harij", "Radhanpur", "Sami", "Santalpur", "Sarasvati", "Sidhpur"],
          "Porbandar": ["Porbandar", "Kutiyana", "Ranavav"],
          "Rajkot": ["Rajkot", "Dhoraji", "Gondal", "Jam Kandorna", "Jasdan", "Jetpur", "Kotada Sangani", "Lodhika", "Padadhari", "Upleta", "Vinchhiya"],
          ...
          "Surendranagar": ["Surendranagar (Wadhwan)", "Chotila", "Chuda", "Dasada", "Dhrangadhra", "Lakhtar", "Limbdi", "Muli", "Sayla", "Thangadh"],
...
export default function CartPage() {
  ...
  const shippingCost = shippingMethod === "express" ? 250 : subtotal > 2000 ? 0 : 150;
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    if (name === "country") {
        setShippingInfo({ ...shippingInfo, country: value, state: "", district: "", city: "" });
    } else if (name === "state") {
        setShippingInfo({ ...shippingInfo, state: value, district: "", city: "" });
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      // We combine District and City to save it sequentially to `shipping_city` in db
  ...
  const availableStates = shippingInfo.country && GEO_DATA[shippingInfo.country] 
        ? Object.keys(GEO_DATA[shippingInfo.country]) 
        : [];
  const availableDistricts = shippingInfo.country && shippingInfo.state && GEO_DATA[shippingInfo.country][shippingInfo.state] 
        ? Object.keys(GEO_DATA[shippingInfo.country][shippingInfo.state]) 
        : [];
  const availableCities = shippingInfo.country && shippingInfo.state && shippingInfo.district && GEO_DATA[shippingInfo.country][shippingInfo.state][shippingInfo.district] 
        ? GEO_DATA[shippingInfo.country][shippingInfo.state][shippingInfo.district] 
        : [];

  const isGeodataAvailable = shippingInfo.country && GEO_DATA[shippingInfo.country];
          ...
          <section className="pt-32 pb-24 px-4 sm:px-6 lg:px-8">
            <div className="max-w-2xl mx-auto text-center">
              ...
              <p className="text-muted-foreground mb-8">
                Thank you for your order. We've received it and will begin crafting your wooden map soon.
              </p>
              <div className="bg-secondary rounded-lg p-6 mb-8 text-left">
                <p className="text-sm text-muted-foreground mb-2">Order Number</p>
                <p className="font-mono text-lg text-foreground font-bold mb-4">#{confirmedOrderNumber}</p>
          ...
          <div className="max-w-7xl mx-auto">
            {cartItems.length === 0 && step === "cart" ? (
              <div className="text-center py-16">
                ...
                <Button asChild><Link href="/shop">Browse Maps</Link></Button>
              </div>
            ) : (
                ...
                <div className="lg:col-span-2">
                  ...
                  {step === "cart" && (
                    <div className="space-y-6">
                      {cartItems.map((item) => (
                        <Card key={item.cartItemId || item.id} className="border-border">
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex justify-between items-start mb-2">
                                  ...
                                  <Button
                                    ...
                                    onClick={() => removeFromCart(item.cartItemId || `${item.id}-${item.size}-${item.color}`)}
                                ...
                                <div className="flex justify-between items-center mt-4">
                                  <div className="flex items-center gap-2">
                                    <Button variant="outline" size="icon" className="h-8 w-8 bg-transparent"
                                      onClick={() => handleUpdateQuantity(item.cartItemId || `${item.id}-${item.size}-${item.color}`, -1)}>
                                  ...
                                  <p className="font-semibold text-foreground">{formatPrice(item.price * item.quantity, settings.currency)}</p>
                                </div>
                              </div>
                            </div>
                          </CardContent>
                      ...
                      <CardContent className="p-6">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          <div>
                            ...
                            <Input id="firstName" name="firstName" value={shippingInfo.firstName} onChange={handleInputChange} className="mt-1" required />
                          </div>
                          <div>
                            <Label htmlFor="lastName">Last Name *</Label>
                            <Input id="lastName" name="lastName" value={shippingInfo.lastName} onChange={handleInputChange} className="mt-1" required />
                          </div>
                          <div className="sm:col-span-2">
                            <Label htmlFor="email">Email *</Label>
                            <Input id="email" name="email" type="email" value={shippingInfo.email} onChange={handleInputChange} className="mt-1" required />
                          </div>
                          <div className="sm:col-span-2">
                            <Label htmlFor="phone">Phone</Label>
                            <Input id="phone" name="phone" type="tel" value={shippingInfo.phone} onChange={handleInputChange} className="mt-1" />
                          </div>
                          <div className="sm:col-span-2">
                            <Label htmlFor="address">Address *</Label>
                            <Input id="address" name="address" value={shippingInfo.address} onChange={handleInputChange} className="mt-1" required />
                          </div>
                          
                          <div>
                            <Label htmlFor="zip">PIN Code *</Label>
                            <Input id="zip" name="zip" value={shippingInfo.zip} onChange={handleInputChange} className="mt-1" required />
                          </div>
                          ...
                          <div>
                            ...
                            {isGeodataAvailable ? (
                                ...
                                <Input id="state" name="state" value={shippingInfo.state} onChange={handleInputChange} className="mt-1" required />
                            )}
                          </div>
                          ...
                          <div>
                            ...
                            {isGeodataAvailable ? (
                                ...
                                <Input id="city" name="city" value={shippingInfo.city} onChange={handleInputChange} className="mt-1" required />
                            )}
                          </div>
                        ...
                        <div className="mt-8">
                          ...
                          <RadioGroup value={shippingMethod} onValueChange={setShippingMethod} className="space-y-3">
                            <div className="flex items-center justify-between p-4 border border-border rounded-lg cursor-pointer hover:border-muted-foreground">
                              ...
                              <span className="font-medium text-foreground">{subtotal > 2000 ? "Free" : formatPrice(150, settings.currency)}</span>
                            </div>
                            <div className="flex items-center justify-between p-4 border border-border rounded-lg cursor-pointer hover:border-muted-foreground">
                              <div className="flex items-center gap-3">
                              ...
                              <span className="font-medium text-foreground">{formatPrice(250, settings.currency)}</span>
                            </div>
                          </RadioGroup>
                        </div>
                      </CardContent>
                    ...
                    <CardContent className="p-6">
                      ...
                      <div className="space-y-4 text-sm">
                        {cartItems.map((item) => (
                          <div key={item.cartItemId || item.id} className="flex justify-between">
                            ...
                            <span className="text-foreground">{formatPrice(item.price * item.quantity, settings.currency)}</span>
                          </div>
                        ))}
                        <div className="border-t border-border pt-4">
                          <div className="flex justify-between">
                            <span className="text-muted-foreground">Subtotal</span>
                            <span className="text-foreground">{formatPrice(subtotal, settings.currency)}</span>
                          </div>
                          <div className="flex justify-between mt-2">
                            <span className="text-muted-foreground">Shipping</span>
                            <span className="text-foreground">{shippingCost === 0 ? "Free" : formatPrice(shippingCost, settings.currency)}</span>
                          </div>
                        </div>
                        <div className="border-t border-border pt-4">
                          <div className="flex justify-between items-center">
                            <span className="font-medium text-foreground">Total</span>
                            <span className="text-2xl font-semibold text-foreground">{formatPrice(total, settings.currency)}</span>
                          </div>
                        </div>
                      </div>

                      {step === "cart" && (
                        <Button className="w-full mt-6 bg-[#8b5a3c] hover:bg-[#6d4830]" size="lg" onClick={() => setStep("shipping")}>
                      ...
                      {step === "shipping" && (
                        <div className="space-y-3 mt-6">
                          <Button className="w-full bg-[#8b5a3c] hover:bg-[#6d4830]" size="lg"
                            onClick={() => {
                              if (!shippingInfo.firstName || !shippingInfo.address || !shippingInfo.city || !shippingInfo.country || !shippingInfo.state || !shippingInfo.email) {
                        ...
                        <div className="space-y-3 mt-6">
                          ...
                          >
                            {placingOrder ? (
                              ...
                              <>Place Order — {formatPrice(total, settings.currency)}</>
                            )}
                          </Button>
