  ...
  return (
    <div className="min-h-screen bg-[#fffaf3]">
      <header className="bg-[#f7f1e8] shadow-sm sticky top-0 z-10 border-b border-[#e6dcd0]">
        <div className="px-4 lg:px-8 py-4 flex items-center justify-between">
          ...
          <Button onClick={() => { resetForm(); setShowForm(!showForm); }}
            className="gap-2 bg-[#5a3726] hover:bg-[#3b2412] text-white">
      ...
      <main className="px-4 lg:px-8 py-8 max-w-6xl mx-auto">
        ...
        {/* Add / Edit Form */}
        {showForm && (
          <Card className="mb-8 border-[#e6dcd0]">
            <CardHeader>
              <CardTitle className="font-serif text-[#2b1a12]">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  ...
                  <div>
                    ...
                    <select id="size" value={formData.size}
                      ...
                      className="w-full h-10 px-3 py-2 text-sm rounded-md border border-[#d4b896] bg-white text-[#2b1a12]">
                      <option value="small">Small (30x20 cm)</option>
                      <option value="medium">Medium (60x40 cm)</option>
                      <option value="large">Large (90x60 cm)</option>
                      <option value="xl">Extra Large (120x80 cm)</option>
                    </select>
                  </div>
            ...
            {loading ? (
              ...
              <div className="space-y-4">
                {products.map((product) => (
                  <div key={product.id}
                    className="flex items-center gap-4 p-4 rounded-lg border border-[#e6dcd0] bg-[#fdfaf6] hover:shadow-sm transition-shadow">
                    {/* Image */}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm text-[#5a3726]">
                        ₹{product.price} · Stock: {product.stock} · {product.category} · {product.size}
                      </p>
                    </div>
                    ...
                    <div className="flex gap-2 shrink-0">
                      <Button size="sm" variant="outline" onClick={() => handleEdit(product)}
                        className="border-[#d4b896] text-[#5a3726] gap-1">
